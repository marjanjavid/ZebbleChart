...
namespace Zebble
{
    partial class zebbleOxyPlot
    {
        ...
        public abstract class Axis : PlotElement
        {
            ...
            public static DataPoint InverseTransform(ScreenPoint pp, Axis xaxis, Axis yaxis)
            {
                return xaxis.InverseTransform(pp.x, pp.y, yaxis);
            }
            ...
            public string FormatValue(double xx)
            {
                if (this.LabelFormatter != null)
                {
                    return this.LabelFormatter(xx);
                }

                return this.FormatValueOverride(xx);
            }
            ...
            public virtual object GetValue(double xx)
            {
                return xx;
            }
            ...
            /// <param name="yaxis">The y-axis.</param>
            /// <returns>The data point.</returns>
            public virtual DataPoint InverseTransform(double xx, double yy, Axis yaxis)
            {
                return new DataPoint(this.InverseTransform(xx), yaxis != null ? yaxis.InverseTransform(yy) : 0);
            }
...
#pragma warning restore 1718
            ...
            public virtual ScreenPoint Transform(double xx, double yy, Axis yaxis)
            {
                if (yaxis == null)
                {
                ...
                return new ScreenPoint(this.Transform(xx), yaxis.Transform(yy));
            }
            ...
            public virtual double Transform(double xx)
            {
                return (xx - this.offset) * this.scale;
            }
            ...
            public virtual void Zoom(double newScale)
            {
                ...
                var sgn = Math.Sign(this.scale);
                var mid = (this.ActualMaximum + this.ActualMinimum) / 2;
                ...
                else
                {
                    if (newMinimum < this.AbsoluteMinimum)
                    {
                        var ddd = newMaximum - newMinimum;
                        newMinimum = this.AbsoluteMinimum;
                        newMaximum = this.AbsoluteMinimum + ddd;
                        if (newMaximum > this.AbsoluteMaximum)
                        {
                            newMaximum = this.AbsoluteMaximum;
                        }
                    ...
                    {
                        var dd = newMaximum - newMinimum;
                        newMaximum = this.AbsoluteMaximum;
                        newMinimum = this.AbsoluteMaximum - dd;
                        if (newMinimum < this.AbsoluteMinimum)
                        {
                            newMinimum = this.AbsoluteMinimum;
                        }
            ...
            {
                var sx = (this.Transform(this.ActualMaximum) + this.Transform(this.ActualMinimum)) * 0.5;
            ...
            protected virtual double PostInverseTransform(double xx)
            {
                return xx;
            }
            ...
            /// See <see cref="LogarithmicAxis" /> for examples on how to implement this.</remarks>
            protected virtual double PreTransform(double xx)
            {
                return xx;
            }
