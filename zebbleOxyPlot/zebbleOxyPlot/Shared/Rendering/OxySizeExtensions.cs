...
namespace Zebble
{
        ...
        public static class OxySizeExtensions
        {
            ...
            public static OxyRect GetBounds(this OxySize bounds, double angle, HorizontalAlignment horizontalAlignment, VerticalAlignment verticalAlignment)
            {
                ...
                var xx = Math.Min(Math.Min(q0.X, q1.X), Math.Min(q2.X, q3.X));
                var yy = Math.Min(Math.Min(q0.Y, q1.Y), Math.Min(q2.Y, q3.Y));
                var ww = Math.Max(Math.Max(q0.X - xx, q1.X - xx), Math.Max(q2.X - xx, q3.X - xx));
                var hh = Math.Max(Math.Max(q0.Y - yy, q1.Y - yy), Math.Max(q2.Y - yy, q3.Y - yy));

                return new OxyRect(xx, yy, ww, hh);
            }
