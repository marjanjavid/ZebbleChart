...
namespace Zebble
{
    partial class zebbleOxyPlot
    {
        ...
        public class CohenSutherlandClipping
        {
            ...
            public bool ClipLine(ref ScreenPoint p0, ref ScreenPoint p1)
            {
                ...
                while (true)
                {
                    ...
                    if ((outcodeOut & Top) != 0)
                    {
                        ...
                        xx = p0.x + ((p1.x - p0.x) * (this.ymax - p0.y) / (p1.y - p0.y));
                        yy = this.ymax;
                    }
                    else if ((outcodeOut & Bottom) != 0)
                    {
                        // point is below the clip rectangle
                        xx = p0.x + ((p1.x - p0.x) * (this.ymin - p0.y) / (p1.y - p0.y));
                        yy = this.ymin;
                    }
                    else if ((outcodeOut & Right) != 0)
                    {
                        // point is to the right of clip rectangle
                        yy = p0.y + ((p1.y - p0.y) * (this.xmax - p0.x) / (p1.x - p0.x));
                        xx = this.xmax;
                    }
                    else if ((outcodeOut & Left) != 0)
                    {
                        // point is to the left of clip rectangle
                        yy = p0.y + ((p1.y - p0.y) * (this.xmin - p0.x) / (p1.x - p0.x));
                        xx = this.xmin;
                    }
            ...
            public bool IsInside(ScreenPoint ss)
            {
                if (ss.x < this.xmin)
                {
                    return false;
