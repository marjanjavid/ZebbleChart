        ...
        public static class MathRenderingExtensions
        {
                ...
                bool measure)
            {
                ...
                if (text.Contains("^{") || text.Contains("_{"))
                {
                    ...
                    // Measure
                    var size = InternalDrawMathText(rc, xx, yy, 0, 0, text, textColor, fontFamily, fontSize, fontWeight, true, angle);
                    ...
                    InternalDrawMathText(rc, xx, yy, dx, dy, text, textColor, fontFamily, fontSize, fontWeight, false, angle);
                ...
                double angle)
            {
                ...
                Func<double, double, string, double, OxySize> drawText = (xb, yb, text, fSize) =>
                {
                    if (!measureOnly)
                    {
                        var xr = xx + ((xb - xx + dx) * cosAngle) - ((yb - yy + dy) * sinAngle);
                        var yr = yy + ((xb - xx + dx) * sinAngle) + ((yb - yy + dy) * cosAngle);
                ...
                {
                    // Superscript
                    if (ii + 1 < ss.Length && ss[ii] == '^' && ss[ii + 1] == '{')
                    {
                        var i1 = ss.IndexOf('}', ii);
                        if (i1 != -1)
                        {
                            var supString = ss.Substring(ii + 2, i1 - ii - 2);
                            ii = i1 + 1;
                            var sx = currentX;
                    ...
                    // Subscript
                    if (ii + 1 < ss.Length && ss[ii] == '_' && ss[ii + 1] == '{')
                    {
                        var i1 = ss.IndexOf('}', ii);
                        if (i1 != -1)
                        {
                            var subString = ss.Substring(ii + 2, i1 - ii - 2);
                            ii = i1 + 1;
                            var sx = currentX;
                    ...
                    // Regular text
                    var i2 = ss.IndexOfAny("^_".ToCharArray(), ii + 1);
                    string regularString;
                    if (i2 == -1)
                    {
                        regularString = ss.Substring(ii);
                        ii = ss.Length;
                    }
                    else
                    {
                        regularString = ss.Substring(ii, i2 - ii);
                        ii = i2;
                    }
