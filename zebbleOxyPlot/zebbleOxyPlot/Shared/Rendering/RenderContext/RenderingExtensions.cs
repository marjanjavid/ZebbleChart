        ...
        public static class RenderingExtensions
        {
            ...
            */
            ...
            public static void DrawImage(
                ...
                bool interpolate)
            {
                rc.DrawImage(image, 0, 0, image.Width, image.Height, xx, yy, ww, hh, opacity, interpolate);
            }
            ...
            public static void DrawClippedImage(
                ...
                bool interpolate)
            {
                if (xx > clippingRectangle.Right || xx + ww < clippingRectangle.Left || yy > clippingRectangle.Bottom || yy + hh < clippingRectangle.Top)
                {
                    return;
                }
                ...
                if (rc.SetClip(clippingRectangle))
                {
                    ...
                    rc.DrawImage(source, xx, yy, ww, hh, opacity, interpolate);
                    rc.ResetClip();
                    return;
                }

                // Fint the positions of the clipping rectangle normalized to image coordinates (0,1)
                var i0 = (clippingRectangle.Left - xx) / ww;
                var i1 = (clippingRectangle.Right - xx) / ww;
                var j0 = (clippingRectangle.Top - yy) / hh;
                var j1 = (clippingRectangle.Bottom - yy) / hh;
                ...
                var destx = i0 < 0 ? xx : xx + (srcx / source.Width * ww);
                var desty = j0 < 0 ? yy : yy + (srcy / source.Height * hh);
                var destw = ww * srcw / source.Width;
                var desth = hh * srch / source.Height;
                ...
                int n = 100)
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    var aa = Math.PI * 2 * i / (n - 1);
                    points[i] = new ScreenPoint(cx + (rx * Math.Cos(aa)), cy + (ry * Math.Sin(aa)));
                }
            ...
            /// <param name="stroke">The stroke color.</param>
            /// <param name="thickness">The thickness.</param>
            public static void DrawCircle(this IRenderContext rc, double xx, double yy, double rr, OxyColor fill, OxyColor stroke, double thickness = 1)
            {
                rc.DrawEllipse(new OxyRect(xx - rr, yy - rr, rr * 2, rr * 2), fill, stroke, thickness);
            }
            ...
            /// <param name="stroke">The stroke color.</param>
            /// <param name="thickness">The thickness.</param>
            public static void DrawCircle(this IRenderContext rc, ScreenPoint center, double rr, OxyColor fill, OxyColor stroke, double thickness = 1)
            {
                DrawCircle(rc, center.X, center.Y, rr, fill, stroke, thickness);
            }
            ...
            /// <param name="r">The radius.</param>
            /// <param name="fill">The fill color.</param>
            public static void FillCircle(this IRenderContext rc, ScreenPoint center, double rr, OxyColor fill)
            {
                DrawCircle(rc, center.X, center.Y, rr, fill, OxyColors.Undefined, 0d);
            }
                ...
                IList<ScreenPoint> lines)
            {
                if (type == MarkerType.Custom)
                {
                    ...
                    var poly = outline.Select(o => new ScreenPoint(pp.X + (o.x * size), pp.Y + (o.y * size))).ToList();
                    polygons.Add(poly);
                    return;
                }
                ...
                switch (type)
                {
                    case MarkerType.Circle:
                        {
                            ellipses.Add(new OxyRect(pp.x - size, pp.y - size, size * 2, size * 2));
                            break;
                        }

                    case MarkerType.Square:
                        {
                            rects.Add(new OxyRect(pp.x - size, pp.y - size, size * 2, size * 2));
                            break;
                        }
                    ...
                    case MarkerType.Diamond:
                        {
                            polygons.Add(
                                new[]
                                    {
                                    new ScreenPoint(pp.x, pp.y - (M2 * size)), new ScreenPoint(pp.x + (M2 * size), pp.y),
                                    new ScreenPoint(pp.x, pp.y + (M2 * size)), new ScreenPoint(pp.x - (M2 * size), pp.y)
                                    });
                            break;
                        }
                    ...
                    case MarkerType.Triangle:
                        {
                            polygons.Add(
                                new[]
                                    {
                                    new ScreenPoint(pp.x - size, pp.y + (M1 * size)),
                                    new ScreenPoint(pp.x + size, pp.y + (M1 * size)), new ScreenPoint(pp.x, pp.y - (M2 * size))
                                    });
                            break;
                        }
                    ...
                    case MarkerType.Star:
                        {
                            lines.Add(new ScreenPoint(pp.x - size, pp.y));
                            lines.Add(new ScreenPoint(pp.x + size, pp.y));
                            lines.Add(new ScreenPoint(pp.x, pp.y - size));
                            lines.Add(new ScreenPoint(pp.x, pp.y + size));
                            break;
                        }
                ...
                switch (type)
                {
                    ...
                    case MarkerType.Star:
                        {
                            lines.Add(new ScreenPoint(pp.x - (size * M3), pp.y - (size * M3)));
                            lines.Add(new ScreenPoint(pp.x + (size * M3), pp.y + (size * M3)));
                            lines.Add(new ScreenPoint(pp.x - (size * M3), pp.y + (size * M3)));
                            lines.Add(new ScreenPoint(pp.x + (size * M3), pp.y - (size * M3)));
                            break;
